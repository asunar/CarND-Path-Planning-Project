		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2')
		{
			...
			if (s != "")
			{
				...
				if (event == "telemetry")
				{
					...
					// previous_path: The last path the car was following
					// Previous list of points are useful when doing the transitions
				...
				if (too_close)
				{
					...
					for(int i = 0; i < potentialLanes.size(); i++){
						int currentLane = potentialLanes[i];
						std::cout << "Checking Potential Lane:" << "\t";
						std::cout << currentLane << "\n";						
						bool isSafeToChangeToThisLane = false;
						for(int j = 0; j < sensor_fusion.size(); j++){
							float current_car_d = sensor_fusion[j][6];
							if(isCarInLane(current_car_d, currentLane)){
								//There is a car in the lane we may shift to

								double current_vx = sensor_fusion[j][3];
								double current_vy = sensor_fusion[j][4];
								double current_check_speed = sqrt(current_vx * current_vx + current_vy * current_vy);
								double current_check_car_s = sensor_fusion[j][5];

								current_check_car_s += ((double)prev_size * .02 * current_check_speed);								
								if(isCarFarEnough(current_check_car_s, car_s)){
									std::cout << "Check car s" << "\t";
									std::cout << current_check_car_s << "\n";									
									std::cout << "Ego car s" << "\t";
									std::cout << car_s << "\n";									
									std::cout << "Check car d" << "\t";
									std::cout << current_car_d << "\n";									

									std::cout << "Verdict: Safe to change lane" << "\n";
									std::cout << "****************" << "\n";									
						...
						// Iterated over all cars in sensor fusion
						if(isSafeToChangeToThisLane == true){
							lane = currentLane;
							std::cout << "Changing lane to" << "\t";
							std::cout << currentLane << "\n";
							changedLane = true;
					...
					if(changedLane){
						std::cout << "Changed lane, speeding up " << endl;
						if (ref_vel < 49.5)
						{
							ref_vel += .224;
						}
						...
						std::cout << "Keeping lane, slowing down" << endl;
						ref_vel -= .224;
					}
				...
				// We will reference either the starting point (where the car is) or at the previous path's endpoint
				double ref_x = car_x;
				double ref_y = car_y;
